...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
  (begin
    ...
    (mint-priv token-id amount recipient)
		(print {type: "sft_mint_xzest", payload: { key: {token-id: token-id, recipient: recipient },  amount: amount }})
		(ok true)
	)
)
...
(define-public (burn (token-id uint) (amount uint) (owner principal))
  (begin
    ...
    (burn-priv token-id amount owner)
		(print {type: "sft_burn_xzest", payload: { key: {token-id: token-id, owner: owner },  amount: amount }})
    (ok true)
	)
)
