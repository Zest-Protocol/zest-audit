...
(define-public (withdraw-rewards (token-id uint) (caller principal))
  (let (
    ...
    (print { type: "withdraw-rewards", payload:  { token-id: token-id , owner: caller, withdrawn-funds: total } })
...
(define-public (add-rewards (token-id uint) (delta uint))
  (let (
    ...
    (print { type: "add-rewards", payload:  { added-points: added-points } })
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
    ...
    (mint-priv token-id amount recipient)
		(print { type: "sft_mint_lp_token", payload: { key: { token-id: token-id, recipient: recipient }, amount: amount } })
		(ok true)
	)
)
...
(define-public (burn (token-id uint) (amount uint) (owner principal))
 (begin
    ...
    (burn-priv token-id amount owner)
		(print { type: "sft_burn_lp_token", payload: { key: { token-id: token-id, owner: owner }, amount: amount } })
    (ok true)
	)
)
...
(define-public (recognize-losses (token-id uint) (caller principal))
  (let (
    ...
    (total (+ losses (get-recognized-losses token-id caller)))
  )
    (try! (is-approved-contract contract-caller))
    (map-set recognized-losses { token-id: token-id, owner: caller } total)
    (print { type: "recognize-losses-lp-token", payload: { token-id: token-id, owner: caller, losses: total } })
    (ok losses)
  )
)
...
(define-public (distribute-losses (token-id uint) (delta uint))
  (let (
    ...
    (print { type: "distribute-losses-lp-token", payload: { token-id: token-id,  added-losses: added-losses } })
...
(define-public (set-contract-owner (owner principal))
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) ERR_UNAUTHORIZED)
    (print { type: "set-contract-owner-lp-token", payload: owner })
