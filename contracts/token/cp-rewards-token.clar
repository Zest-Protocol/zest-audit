;; holds BTC rewards to Cover Providers
...
(define-fungible-token cp-rewards-001)
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance cp-rewards-001 who))
)
...
(define-read-only (get-overall-supply)
	(ok (ft-get-supply cp-rewards-001))
)
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
  (let
    ...
    (map-set points-correction { token-id: token-id, owner: recipient } points-correction-to)
		...
		(try! (ft-transfer? cp-rewards-001 amount sender recipient))
...
(define-public (withdraw-rewards (token-id uint) (caller principal))
  (let (
    ...
    (print { type: "withdraw-rewards-cp-rewards-token", payload: { withdrawn-funds: total, token-id: token-id, owner: caller } })
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
  (begin
    (try! (is-approved-contract contract-caller))
		(try! (ft-mint? cp-rewards-001 amount recipient))
		(set-balance token-id (+ (get-balance-uint token-id recipient) amount) recipient)
		(map-set token-supplies token-id (+ (unwrap-panic (get-total-supply token-id)) amount))
    (mint-priv token-id amount recipient)
		(print { type: "sft_mint_cp_rewards_token",  payload: { key: { token-id: token-id, recipient: recipient }, amount: amount }})
		(ok true)
	)
)
...
(define-public (burn (token-id uint) (amount uint) (owner principal))
  (begin
    (try! (is-approved-contract contract-caller))
    (try! (ft-burn? cp-rewards-001 amount owner))
    (set-balance token-id (- (get-balance-uint token-id owner) amount) owner)
		(map-set token-supplies token-id (- (unwrap-panic (get-total-supply token-id)) amount))
    (burn-priv token-id amount owner)
		(print { type: "sft_burn_cp_rewards_token", payload: { key: { token-id: token-id, owner: owner }, amount: amount }})
    (ok true)
	)
)
...
;; ERROR START 10000
(define-constant ERR_INVALID_PRINCIPAL (err u10000))
(define-constant ERR_INSUFFICIENT_BALANCE (err u10001))
(define-constant ERR_PANIC (err u10002))
