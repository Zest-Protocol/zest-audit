...
(define-data-var pool
  { sp-contract: principal, status: (buff 1) }
  { sp-contract: .sp-token, status: INIT })
...
;; @restricted pool
;; @param sp-token: principal of the xZest-token used to account for zest rewards
...
;; @desc add rewards to distribute to stakers
;; @restricted contract-owner
;; @param sp-token: principal of the xZest-token used to account for zest rewards
;; @param amount: amount being distributed to stakers
;; @param caller: principal of account sending funds
;; @returns (response true uint)
...
;; @desc caller withdraws rewards
;; @param sp-token: principal of the xZest-token used to account for zest rewards
;; @param token-id: pool id
;; @param recipient: principal of account receiving rewards
...
;; @param sp-token: token contract that points to the requested pool
;; @param caller: principal of account receiving rewards
;; @returns (response true uint)
...
;; @param sp-token: token contract that points to the requested pool
;; @param amount: amount being sent from the protocol
;; @param caller: principal of account receiving funds
;; @returns (response true uint)
