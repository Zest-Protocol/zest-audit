...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let bridgeId_LP_1 = (consumeUint(block.receipts[0].result.expectOk()));
    ...
    let txSendFunds1 = generateP2SHTx(senderPubKeyLp1, recipient, defaultExpiration, sendFundsHash1, bridgeId_LP_1, sentAmount_1);
    ...
    block = chain.mineBlock([
      Tx.contractCall(
        ...
        "send-funds",
        [
          // Data used by the Magic Bridge contract to prove that the Bitcoin transaction happened
          types.tuple({
          ...
          types.buff(Buffer.from(recipient, "hex")),                        // Public key used by the bridge that is the recipient of Bitcoin.
          ...
          types.buff(Buffer.from(swapperBuff(bridgeId_LP_1), "hex")),       // Swapper id in Little ending padded to 4 bytes (string length of 8). Ex for u0: "00000000"
    ...
    const bridgeId_LP_2 = 1;
    const bridgeId_LP_3 = 2;
    const bridgeId_LP_4 = 3;
    ...
    block = chain.mineBlock([
      ...
      Bridge.initializeSwapper(LP_3.address),
      Bridge.initializeSwapper(LP_4.address),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId0,LP_2.address,senderPubKeyLp2,recipient,defaultExpiration,bridgeId_LP_2,sentAmount_2,sendFundsPreimage2,supplierId0,sentAmount_2 * inboundFee / 10_000,commitmentTime2,chain.blockHeight - 1),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId1,LP_2.address,senderPubKeyLp2,recipient,defaultExpiration,bridgeId_LP_2,sentAmount_2,sendFundsPreimage3,supplierId0,sentAmount_2 * inboundFee / 10_000,commitmentTime2,chain.blockHeight - 1),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId2,LP_3.address,senderPubKeyLp3,recipient,defaultExpiration,bridgeId_LP_3,sentAmount_3,sendFundsPreimage4,supplierId0,sentAmount_3 * inboundFee / 10_000,commitmentTime3,chain.blockHeight - 1),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId0,LP_4.address,senderPubKeyLp4,recipient,defaultExpiration,bridgeId_LP_4,sentAmount_4,sendFundsPreimage4,supplierId0,sentAmount_4 * inboundFee / 10_000,commitmentTime4,chain.blockHeight - 1),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId3,LP_1.address,senderPubKeyLp1,recipient,defaultExpiration,bridgeId_LP_1,sentAmount_5,generateRandomHexString(20),supplierId0,sentAmount_5 * inboundFee / 10_000,commitmentTime5,chain.blockHeight - 1),
      ...sendFundsP2SHTxs(deployerWallet.address,poolId3,LP_2.address,senderPubKeyLp2,recipient,defaultExpiration,bridgeId_LP_2,sentAmount_6,generateRandomHexString(20),supplierId0,sentAmount_6 * inboundFee / 10_000,commitmentTime6,chain.blockHeight - 1),
    ]);
    ...
    block = chain.mineBlock([
      ...sendFundsP2SHTxs(deployerWallet.address,poolId0,LP_2.address,senderPubKeyLp2,recipient,defaultExpiration,bridgeId_LP_2,sentAmount_2,sendFundsPreimage5,supplierId0,sentAmount_2 * inboundFee / 10_000,commitmentTime2,chain.blockHeight - 1),
    ]);
...
000000000001976a914c5b8cc55ab829cc07b08936234b14a039a38ffe288ac89692200`
    ...
    block = chain.mineBlock([ Bridge.initializeSwapper(borrower_1.address) ]);
    let borrower_1_bridge_id = consumeUint(block.receipts[0].result.expectOk());
    block = chain.mineBlock([ Bridge.initializeSwapper(borrower_2.address) ]);
    let borrower_2_bridge_id = consumeUint(block.receipts[0].result.expectOk());
    block = chain.mineBlock([ Bridge.initializeSwapper(borrower_3.address) ]);
    let borrower_3_bridge_id = consumeUint(block.receipts[0].result.expectOk());
    
    let paymentPreimage1 = generateRandomHexString(20);

    block = chain.mineBlock([
      ...makePaymentTxs(deployerWallet.address, borrower_1.address,sender_1,recipient,defaultExpiration,borrower_1_bridge_id,paymentLoans[loan2],paymentPreimage1,supplierId0,paymentLoansMinToReceive[loan2],loan2, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId0, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_1.address,sender_1,recipient,defaultExpiration,borrower_1_bridge_id,paymentLoans[loan3],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan3],loan3, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId0, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      // fails to make a payment because is not active
      ...makePaymentTxs(deployerWallet.address, borrower_2.address,sender_2,recipient,defaultExpiration,borrower_2_bridge_id,paymentLoans[loan4],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan4],loan4, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId1, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_2.address,sender_2,recipient,defaultExpiration,borrower_2_bridge_id,paymentLoans[loan5],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan5],loan5, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoans[loan6],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan6],loan6, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoans[loan7],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan7],loan7, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
    ]);

    block = chain.mineBlock([
      // fails to make a payment because is not active
      ...makePaymentTxs(deployerWallet.address, borrower_1.address,sender_1,recipient,defaultExpiration,borrower_1_bridge_id,paymentLoans[loan8],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan8],loan8, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_1.address,sender_1,recipient,defaultExpiration,borrower_1_bridge_id,paymentLoans[loan9],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan9],loan9, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId0, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_2.address,sender_2,recipient,defaultExpiration,borrower_2_bridge_id,paymentLoans[loan10],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan10],loan10, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId1, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoans[loan11],generateRandomHexString(20),supplierId0,paymentLoansMinToReceive[loan11],loan11, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
    ]);
    ...
    block = chain.mineBlock([
      ...makePaymentTxs(deployerWallet.address, borrower_1.address,sender_1,recipient,defaultExpiration,borrower_1_bridge_id,repaymentLoan3,generateRandomHexString(20),supplierId0,getBP(repaymentLoan3, inboundFee),loan3, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId1, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
      ...makePaymentTxs(deployerWallet.address, borrower_2.address,sender_2,recipient,defaultExpiration,borrower_2_bridge_id,repaymentLoan10,generateRandomHexString(20),supplierId0,getBP(repaymentLoan10, inboundFee),loan10, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId1, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC),
    ]);
...
000000000001976a914c5b8cc55ab829cc07b08936234b14a039a38ffe288ac89692200`
    ...
    // Loan_3 payment
    block = chain.mineBlock([...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoan13,generateRandomHexString(20),supplierId0,paymentLoansMinToReceive13,loan13, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC)]);
    chain.mineEmptyBlockUntil(consumeUint(loan.getLoanData(loan13).result.expectTuple()["next-payment"]) - 1);
    block = chain.mineBlock([...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoan13,generateRandomHexString(20),supplierId0,paymentLoansMinToReceive13,loan13, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC)]);
    chain.mineEmptyBlockUntil(consumeUint(loan.getLoanData(loan13).result.expectTuple()["next-payment"]) - 1);
    block = chain.mineBlock([...makePaymentTxs(deployerWallet.address, borrower_3.address,sender_3,recipient,defaultExpiration,borrower_3_bridge_id,paymentLoan13,generateRandomHexString(20),supplierId0,paymentLoansMinToReceive13,loan13, PAYMENT, LP_TOKEN,LIQUIDITY_VAULT,poolId2, CP_TOKEN, CP_REWARDS_TOKEN, ZP_TOKEN, SWAP_ROUTER,chain.blockHeight - 1, XBTC)]);
