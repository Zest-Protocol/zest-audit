...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block = chain.mineBlock([
      ...
      Bridge.initializeSwapper(wallet_2.address),
    ...
    ZestRewardDist.getCycleSharePrincipal(0, 1, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 2, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 3, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 4, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 5, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 6, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 7, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
    ZestRewardDist.getCycleSharePrincipal(0, 8, wallet_1.address, chain, deployerWallet.address).result.expectUint(300_000_000);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let res = consumeUint(ZestRewardDist.getWithdrawableRewards(0, wallet_1.address, chain, deployerWallet.address).result.expectTuple()["cycle-rewards"]);
    block = pool.withdrawZestRewards(0,ZP_TOKEN, REWARDS_CALC, wallet_1.address);

    let rewardsFactor = consumeUint(RewardsCalc.getMultiplier(chain, REWARDS_CALC, 8, wallet_1.address));
    let outcome = getBP(res, rewardsFactor);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let res = (consumeUint(ZestRewardDist.getallCycleRewardsOf(0, wallet_1.address, chain, deployerWallet.address).result));
    block = pool.withdrawZestRewards(0,ZP_TOKEN, REWARDS_CALC, wallet_1.address);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let rewardsFactor = consumeUint(RewardsCalc.getMultiplier(chain, REWARDS_CALC, 8, wallet_1.address));
    let outcome = getBP(sum, rewardsFactor);

    let commitment = (consumeUint(ZestRewardDist.getBalanceUint(0, wallet_1.address, chain, deployerWallet.address).result));
    let delta = 2;
    let passiveRewards = getBP(commitment * delta / PASSIVE_DIVIDER, PASSIVE_BPS);

    assertEquals(chain.getAssetsMaps().assets[".zge000-governance-token.zest"]["ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5"], (outcome + passiveRewards));
  },
});
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    assertEquals(chain.getAssetsMaps().assets[".zge000-governance-token.zest"]["ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5"] - previous, passiveRewards);
  },
});
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let multiplier = consumeUint(RewardsCalc.getMultiplier(chain, "rewards-calc", 8, deployerWallet.address));

    let funds = (consumeUint(lpToken.getBalance("lp-token", 0, wallet_1.address).result.expectOk() as string));

    assertEquals(chain.getAssetsMaps().assets[".zge000-governance-token.zest"][wallet_1.address], outcome + passiveRewards);
    ZestRewardDist.getCycleSharePrincipal(0, 9, wallet_1.address, chain, deployerWallet.address).result.expectUint(0);
    ZestRewardDist.getCycleSharePrincipal(0, 10, wallet_1.address, chain, deployerWallet.address).result.expectUint(funds);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let rewardsFactor = consumeUint(RewardsCalc.getMultiplier(chain, REWARDS_CALC, 6, wallet_1.address));
    let outcome = getBP(sum, rewardsFactor);

    commitment = (consumeUint(ZestRewardDist.getBalanceUint(0, wallet_1.address, chain, deployerWallet.address).result));

    delta = 8;
    passiveRewards = getBP(commitment * delta / PASSIVE_DIVIDER, PASSIVE_BPS);

    block = pool.withdrawZestRewards(0,ZP_TOKEN, REWARDS_CALC, wallet_1.address);
    block.receipts[0].result.expectOk().expectTuple()["zest-base-rewards"].expectUint(passiveRewards);
    ...
    prev  = 0;
    sum = (consumeUint(ZestRewardDist.getallCycleRewardsOf(0, wallet_2.address, chain, deployerWallet.address).result));
    rewardsFactor = consumeUint(RewardsCalc.getMultiplier(chain, REWARDS_CALC, 8, wallet_2.address));
    outcome = getBP(sum, rewardsFactor);

    commitment = (consumeUint(ZestRewardDist.getBalanceUint(0, wallet_2.address, chain, deployerWallet.address).result));
    delta = 9;
    passiveRewards = getBP(commitment * delta / PASSIVE_DIVIDER, PASSIVE_BPS);
